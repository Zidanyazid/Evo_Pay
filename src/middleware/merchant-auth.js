...
export async function merchantAuth(req, res, next) {
  ...
  const merchant = await db.get('SELECT * FROM merchants WHERE api_key_hash=? AND is_active=1', [hashApiKey(token)]);
  if (!merchant) return res.status(401).json({ error: { code: 'UNAUTHORIZED', message: 'API key tidak valid.' } });
  const access = await ipAllowlistService.evaluate(merchant, req.ip);
  ...
  const signed = await verifySignedRequest(req, merchant);
