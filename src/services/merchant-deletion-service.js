...
export async function deleteMerchant({ merchantId,confirmationName,actorId=null,ip=null,userAgent=null }) {
  ...
  const history=Object.fromEntries(await Promise.all(historyTables.map(async(table)=>[table,(await db.get(`SELECT COUNT(*) count FROM ${table} WHERE merchant_id=?`,[merchantId])).count])));
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.run('INSERT INTO audit_logs (id,actor_id,action,target_type,target_id,ip,user_agent,metadata_json,created_at) VALUES (?,?,?,?,?,?,?,?,?)',[id('aud'),actorId,'MERCHANT_DELETED','merchant',merchant.id,ip,userAgent,JSON.stringify({name:merchant.name}),now()]);
  });
