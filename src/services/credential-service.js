...
export class CredentialService {
  list(merchantId) { return db.prepare('SELECT * FROM api_credentials WHERE merchant_id=? ORDER BY created_at DESC').all(merchantId).map(publicCredential); }

  create({ merchantId, name, environment = 'sandbox', scopes = ['payments:read', 'payments:write'], expiresAt = null, rotationParentId = null, overlapEndsAt = null }) {
    ...
    if (environment === 'live') {
      const merchant = db.prepare('SELECT onboarding_status FROM merchants WHERE id=?').get(merchantId);
    ...
    db.prepare('INSERT INTO api_credentials (id,merchant_id,name,key_prefix,key_hash,secret_encrypted,environment,scopes_json,expires_at,rotation_parent_id,overlap_ends_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(row.id,row.merchantId,row.name,row.prefix,hashApiKey(secret),row.secretEncrypted,row.environment,row.scopes,row.expiresAt,row.rotationParentId,row.overlapEndsAt,row.createdAt);
    return { ...publicCredential(db.prepare('SELECT * FROM api_credentials WHERE id=?').get(row.id)), secret };
  }

  rotate(credentialId, overlapHours = 24) {
    const old = db.prepare('SELECT * FROM api_credentials WHERE id=? AND revoked_at IS NULL').get(credentialId);
    ...
    db.prepare('UPDATE api_credentials SET overlap_ends_at=? WHERE id=?').run(overlapEndsAt, old.id);
  ...
  revoke(credentialId) {
    const result = db.prepare('UPDATE api_credentials SET revoked_at=? WHERE id=? AND revoked_at IS NULL').run(now(), credentialId);
    if (!result.changes) throw serviceError('Credential aktif tidak ditemukan.', 404);
    return publicCredential(db.prepare('SELECT * FROM api_credentials WHERE id=?').get(credentialId));
  }
}
