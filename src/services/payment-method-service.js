...
export class PaymentMethodService {
  async seedMerchant(merchantId) {
    const timestamp = now();
    await db.transaction(async (tx) => {
      for (const item of PAYMENT_METHODS) {
        await tx.run('INSERT IGNORE INTO merchant_payment_methods (merchant_id,payment_method,is_enabled,fee_bearer,admin_fee_fixed,admin_fee_percentage,settlement_label,minimum_amount,maximum_amount,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [merchantId, item.code, item.defaultEnabled ? 1 : 0, 'CUSTOMER', item.adminFeeFixed, item.adminFeePercentage, item.settlementLabel, item.minimumAmount, item.maximumAmount, timestamp, timestamp]);
        await tx.run("UPDATE merchant_payment_methods SET is_enabled=?,fee_bearer='CUSTOMER',admin_fee_fixed=?,admin_fee_percentage=?,settlement_label=?,minimum_amount=?,maximum_amount=?,updated_at=? WHERE merchant_id=? AND payment_method=? AND admin_fee_fixed=0 AND admin_fee_percentage=0 AND settlement_label='Sesuai akun Tokopay' AND minimum_amount=1000 AND maximum_amount IS NULL", [item.defaultEnabled ? 1 : 0, item.adminFeeFixed, item.adminFeePercentage, item.settlementLabel, item.minimumAmount, item.maximumAmount, timestamp, merchantId, item.code]);
      }
    });
  }

  async seedAll() { const merchants = await db.all('SELECT id FROM merchants'); await Promise.all(merchants.map(({ id }) => this.seedMerchant(id))); }
  async list(merchantId) { await this.seedMerchant(merchantId); const states = new Map((await db.all('SELECT * FROM merchant_payment_methods WHERE merchant_id=?', [merchantId])).map((row) => [row.payment_method, row])); return PAYMENT_METHODS.map((item) => rowToMethod(item, states.get(item.code))); }
  async configuration(merchantId, value) { const code = normalizePaymentMethod(value); const method = paymentMethodByCode(code); if (!method) throw validationError('Metode pembayaran tidak dikenal.', 'PAYMENT_METHOD_UNKNOWN'); await this.seedMerchant(merchantId); return rowToMethod(method, await db.get('SELECT * FROM merchant_payment_methods WHERE merchant_id=? AND payment_method=?', [merchantId, code])); }
  async assertEnabled(merchantId, value, amount = null) { const method = await this.configuration(merchantId, value); if (method.requires_registration) throw validationError(`Metode pembayaran ${method.code} memerlukan pendaftaran merchant.`, 'PAYMENT_METHOD_REGISTRATION_REQUIRED'); if (!method.is_enabled) throw validationError(`Metode pembayaran ${method.code} sedang nonaktif untuk merchant ini.`, 'PAYMENT_METHOD_DISABLED'); if (amount != null && (amount < method.minimum_amount || (method.maximum_amount != null && amount > method.maximum_amount))) throw validationError(`Nominal ${method.code} harus antara Rp${method.minimum_amount.toLocaleString('id-ID')} dan ${method.maximum_amount == null ? 'tanpa batas maksimum' : `Rp${method.maximum_amount.toLocaleString('id-ID')}`}.`, 'PAYMENT_AMOUNT_OUT_OF_RANGE'); return method; }
  async update(merchantId, value, input = {}) {
    const current = await this.configuration(merchantId, value); const bearer = String(input.fee_bearer ?? current.fee_bearer).toUpperCase();
    ...
    await db.run('UPDATE merchant_payment_methods SET is_enabled=?,fee_bearer=?,admin_fee_fixed=?,admin_fee_percentage=?,settlement_label=?,minimum_amount=?,maximum_amount=?,updated_at=? WHERE merchant_id=? AND payment_method=?', [enabled ? 1 : 0, bearer, fixed, percentage, settlement, minimum, maximum, now(), merchantId, current.code]);
