...
export function calculateFee({ merchantId, provider, method, amount, methodConfig=null }) {
  const config=methodConfig||db.prepare('SELECT * FROM merchant_payment_methods WHERE merchant_id=? AND payment_method=?').get(merchantId,method);
  if(config&&(Number(config.admin_fee_fixed||0)>0||Number(config.admin_fee_percentage||0)>0)){
    const fee=Number(config.admin_fee_fixed||0)+Math.round(amount*Number(config.admin_fee_percentage||0)/100),bearer=config.fee_bearer||'MERCHANT';
  ...
  const rule = db.prepare(`SELECT * FROM fee_rules WHERE is_active=1 AND (merchant_id=? OR merchant_id IS NULL) AND (provider=? OR provider IS NULL) AND (payment_method=? OR payment_method IS NULL) ORDER BY merchant_id IS NOT NULL DESC,provider IS NOT NULL DESC,payment_method IS NOT NULL DESC,priority DESC LIMIT 1`).get(merchantId, provider, method);
